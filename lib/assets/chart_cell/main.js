  ...
  const app = Vue.createApp({
    ...
    methods: {
      ...
      handleFieldChange(event) {
        ...
        const value = layer ? this.layers[layer][field] : this.rootFields[field];
        ctx.pushEvent("update_field", { field, value, layer: layer && parseInt(layer) });
      },
      addLayer() {
    ...
    components: {
      ...
      BaseCheckbox: {
        ...
        template: `
          <div :class="[inline ? 'inline-field' : 'field', grow ? 'grow' : '']">
            <label v-if="!inline" class="input-label">{{ label }}</label>
            <input
              type="checkbox"
              :value="modelValue"
              :checked="modelValue"
            ...
            >
            <label v-if="inline" class="inline-input-label">{{ label }}</label>
          </div>
        `,
      },
      Accordion: {
      ...
      FieldSettings: {
        ...
        template: `
          ...
          <div class="field-settings" v-if="isOpen">
            <slot name="content" />
          </div>
        `,
      },
